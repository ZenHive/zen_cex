defmodule ZenCex.Adapters.Binance.RateLimitIntegrationTest do
  ...
  describe "rate limiting with real API (safe tests)" do
    ...
    test "different endpoints have different weights" do
      ...
      # Create mock requests to test weight calculation
      mock_request = %{options: %{url: "/api/v3/ticker/price", method: :get, params: %{}}}
      assert Adapter.calculate_request_weight(mock_request) == 1

      mock_order_request = %{options: %{url: "/api/v3/order", method: :post, params: %{}}}
      assert Adapter.calculate_request_weight(mock_order_request) == 10

      mock_account_request = %{options: %{url: "/api/v3/account", method: :get, params: %{}}}
      assert Adapter.calculate_request_weight(mock_account_request) == 10
    end
  end
